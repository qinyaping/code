 ...
 */
static struct turn_attr_hdr* turn_attr_address_create(uint16_t type,
...
struct turn_msg_hdr* turn_msg_binding_response_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((STUN_METHOD_BINDING | STUN_SUCCESS_RESP), len, id,
			iov);
}

struct turn_msg_hdr* turn_msg_binding_error_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((STUN_METHOD_BINDING | STUN_ERROR_RESP), len, id,
			iov);
}
...
struct turn_msg_hdr* turn_msg_allocate_response_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((TURN_METHOD_ALLOCATE | STUN_SUCCESS_RESP), len, id,
			iov);
}

struct turn_msg_hdr* turn_msg_allocate_error_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((TURN_METHOD_ALLOCATE | STUN_ERROR_RESP), len, id,
			iov);
}
...
struct turn_msg_hdr* turn_msg_refresh_response_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((TURN_METHOD_REFRESH | STUN_SUCCESS_RESP), len, id,
			iov);
}

struct turn_msg_hdr* turn_msg_refresh_error_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((TURN_METHOD_REFRESH | STUN_ERROR_RESP), len, id,
			iov);
}

struct turn_msg_hdr* turn_msg_createpermission_request_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((TURN_METHOD_CREATEPERMISSION | STUN_REQUEST), len,
			id, iov);
}

struct turn_msg_hdr* turn_msg_createpermission_response_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((TURN_METHOD_CREATEPERMISSION | STUN_SUCCESS_RESP),
			len, id, iov);
}

struct turn_msg_hdr* turn_msg_createpermission_error_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((TURN_METHOD_CREATEPERMISSION | STUN_ERROR_RESP),
			len, id, iov);
}

struct turn_msg_hdr* turn_msg_channelbind_request_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((TURN_METHOD_CHANNELBIND | STUN_REQUEST), len, id,
			iov);
}

struct turn_msg_hdr* turn_msg_channelbind_response_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((TURN_METHOD_CHANNELBIND | STUN_SUCCESS_RESP), len,
			id, iov);
}

struct turn_msg_hdr* turn_msg_channelbind_error_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((TURN_METHOD_CHANNELBIND | STUN_ERROR_RESP), len,
			id, iov);
}
...
struct turn_msg_hdr* turn_msg_connect_response_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((TURN_METHOD_CONNECT | STUN_SUCCESS_RESP), len, id,
			iov);
}

struct turn_msg_hdr* turn_msg_connect_error_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((TURN_METHOD_CONNECT | STUN_ERROR_RESP), len, id,
			iov);
}

struct turn_msg_hdr* turn_msg_connectionbind_request_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((TURN_METHOD_CONNECTIONBIND | STUN_REQUEST), len,
			id, iov);
}

struct turn_msg_hdr* turn_msg_connectionbind_response_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((TURN_METHOD_CONNECTIONBIND | STUN_SUCCESS_RESP),
			len, id, iov);
}

struct turn_msg_hdr* turn_msg_connectionbind_error_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((TURN_METHOD_CONNECTIONBIND | STUN_ERROR_RESP), len,
			id, iov);
}

struct turn_msg_hdr* turn_msg_connectionattempt_indication_create(uint16_t len,
		const uint8_t* id, struct iovec* iov) {
	return turn_msg_create((TURN_METHOD_CONNECTIONATTEMPT | STUN_INDICATION),
			len, id, iov);
}
/* STUN attributes */
...
struct turn_attr_hdr* turn_attr_error_create(uint16_t code, const char* reason,
	...
	if (is_little_endian()) {
		ret->turn_attr_reserved_class = class_type << 16;
	} else /* big endian */
	{
		ret->turn_attr_reserved_class = class_type;
	}
...
struct turn_attr_hdr* turn_attr_xor_mapped_address_create(
		const struct sockaddr* address, uint32_t cookie, const uint8_t* id,
		struct iovec* iov) {
	return turn_attr_xor_address_create(STUN_ATTR_XOR_MAPPED_ADDRESS, address,
			cookie, id, iov);
}
...
struct turn_attr_hdr* turn_attr_xor_peer_address_create(
		const struct sockaddr* address, uint32_t cookie, const uint8_t* id,
		struct iovec* iov) {
	return turn_attr_xor_address_create(TURN_ATTR_XOR_PEER_ADDRESS, address,
			cookie, id, iov);
}
...
struct turn_msg_hdr* turn_error_response_400(int method, const uint8_t* id,
	...
	/* error-code */
	if (!(attr = turn_attr_error_create(400, STUN_ERROR_400,
			sizeof(STUN_ERROR_400), &iov[*idx]))) {
...
int turn_xor_address_cookie(int family, uint8_t* peer_addr,
		uint16_t* peer_port, const uint8_t* cookie, const uint8_t* msg_id) {
	size_t i = 0;
	size_t len = 0;
		...
		struct turn_message* message, uint16_t* unknown, size_t* unknown_size)
{
	...
	if (msg_len < 20) // STUN/TURN header MUST be 20 bytes length
	{
		return -1;    // not a STUN/TURN message
	}

	const char* ptr = msg;
	struct turn_msg_hdr* hdr = (struct turn_msg_hdr*) ptr;
//	hdr = (struct turn_msg_hdr*) ptr;
	message->msg = hdr; /* keep pointer */
	ssize_t len = ntohs(hdr->turn_msg_len);

	if ((len + 20) > msg_len) //check if the length coherent with packet length received
	{
		return -1;
	}
	if (len % 4)  //length is a multipe of four
	{
		return -1;
	}
	...
	while (len >= 4)
	{
		...
		// the FINGERPRINT attribute MUST be the last attribute, when present, ignore the following attribute
		if (message->fingerprint)
		{
			return 0;
		}

		// MESSAGE-INTEGRITY is the last attribute except if FINGERPRINT follow it, ignore the following attribute
		...
		size_t padding = (4 + ntohs(attr->turn_attr_len)) % 4;
		if (padding) {
			len -= (4 - padding);
			ptr += (4 - padding);
		}

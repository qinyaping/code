    ...
    const struct sockaddr* client_addr, socklen_t addr_size)
{
  ...
  list_iterate_safe(get, n, list)
  {
    ...
    char buf_server_tuple[128]= {0};
    inet_ntop(AF_INET, &((struct sockaddr_in*)&tmp->tuple.server_addr)->sin_addr, buf_server_tuple, 128);
    int port_server_tuple = ntohs(((struct sockaddr_in*)&tmp->tuple.server_addr)->sin_port);
    char buf_server[128]= {0};
    inet_ntop(AF_INET, &((struct sockaddr_in*)server_addr)->sin_addr, buf_server, 128);
    int port_server = ntohs(((struct sockaddr_in*)server_addr)->sin_port);

     char buf_client_tuple[128]= {0};
     inet_ntop(AF_INET, &((struct sockaddr_in*)&tmp->tuple.client_addr)->sin_addr, buf_client_tuple, 128);
     int port_client_tuple = ntohs(((struct sockaddr_in*)&tmp->tuple.client_addr)->sin_port);

     char buf_client[128]= {0};
     inet_ntop(AF_INET, &((struct sockaddr_in*)client_addr)->sin_addr, buf_client, 128);
     int port_client = ntohs(((struct sockaddr_in*)client_addr)->sin_port);
...
//       !memcmp(&tmp->tuple.client_addr, client_addr, addr_size))
    		!memcmp(buf_server_tuple, buf_server, 128) &&
    		!memcmp(buf_client_tuple, buf_client, 128) &&
    		port_server_tuple == port_server && port_client_tuple == port_client
    		)
    {
      return tmp;
    }
    ...
    const struct sockaddr* relayed_addr, socklen_t addr_size)
{
  ...
  list_iterate_safe(get, n, list)
  {
    ...
    char buf_relay_allocation[128]= {0};
    inet_ntop(AF_INET, &((struct sockaddr_in*)&tmp->relayed_addr)->sin_addr, buf_relay_allocation, 128);
    int port_relay_allocation = ntohs(((struct sockaddr_in*)&tmp->relayed_addr)->sin_port);
    char buf_relay[128]= {0};
    inet_ntop(AF_INET, &((struct sockaddr_in*)relayed_addr)->sin_addr, buf_relay, 128);
    int port_relay = ntohs(((struct sockaddr_in*)relayed_addr)->sin_port);

    if(//!memcmp(&tmp->relayed_addr, relayed_addr, addr_size)
    	!memcmp(buf_relay_allocation, buf_relay, 128) &&
    	port_relay_allocation == port_relay
    )
    {
      return tmp;
    }
